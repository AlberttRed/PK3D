...
func intersect_point(dir, cells=1):
	if weakref(GAME_DATA.ACTUAL_MAP).get_ref(): #Comprovem que l'Acual Map s'hagi actualitzat en el cas de canviar de mapa i aixi evitar que doni error
		...
		return body.get_world().get_direct_space_state().intersect_ray(body.get_translation() + Vector3(0,1,0), body.get_translation()  + Vector3(0,1,0) + (dir*cells+(dir*cells/4)),[body.get_node("StaticBody")])#intersect_point(body.get_translation() + (dir*cells), CONST.GRID_SIZE, get_tree().get_root().get_node("World/CanvasModulate/MapArea_").get_children(), 2147483647, true, true)
...
func intersect_tile(dir, cells=1):
	if weakref(GAME_DATA.ACTUAL_MAP).get_ref(): #Comprovem que l'Acual Map s'hagi actualitzat en el cas de canviar de mapa i aixi evitar que doni error
		...
		return body.get_world().get_direct_space_state().intersect_ray(body.get_translation() + Vector3(0, 2, 0) + dir*cells, body.get_translation() + Vector3(0, -2, 0) +dir*cells,[body.get_node("StaticBody")])#body.get_translation(), body.get_translation() + (dir*cells) + pro)#intersect_point(body.get_translation() + (dir*cells), CONST.GRID_SIZE, get_tree().get_root().get_node("World/CanvasModulate/MapArea_").get_children(), 2147483647, true, true)
